...
pub struct HarnessMetadata {
    ...
    /// Modeling file that was generated by the compiler that includes this harness.
...
impl HarnessAttributes {
    ...
    pub fn is_proof_harness(&self) -> bool {
        matches!(
            self.kind,
            HarnessKind::Proof | HarnessKind::ProofForContract { .. }
        )
    }
}
    ...
    I::Item: Borrow<&'a HarnessMetadata>,
{
    ...
    for md in all_harnesses.into_iter() {
        ...
        if exact_filter {
            ...
            let unqualified_name = md.get_harness_name_unqualified();
            if targets.contains(&md.pretty_name)
                || targets.iter().any(|t| t.as_str() == unqualified_name)
                || targets
                    .iter()
                    .any(|target| md.pretty_name.contains(*target))
            {
                result.push(md);
            } else {
                trace!(skip = md.pretty_name, "find_proof_harnesses");
