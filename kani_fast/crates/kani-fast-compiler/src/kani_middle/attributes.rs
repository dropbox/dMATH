...
impl KaniAttributeKind {
    ...
    pub fn demands_stubbing_use(self) -> bool {
        matches!(
            self,
            KaniAttributeKind::Stub | KaniAttributeKind::StubVerified
        )
    }
}
...
impl<'tcx> KaniAttributes<'tcx> {
    ...
    fn is_contract_generated(&self) -> bool {
        self.map
            .contains_key(&KaniAttributeKind::IsContractGenerated)
    }
        ...
        for (&kind, attrs) in self.map.iter() {
            ...
            match kind {
                ...
                KaniAttributeKind::Proof => {
                    ...
                    attrs
                        .iter()
                        .for_each(|attr| self.check_proof_attribute(kind, attr))
                }
    ...
    pub fn check_unstable_features(&self, enabled_features: &[String]) {
        if !matches!(
            self.tcx.type_of(self.item).skip_binder().kind(),
            TyKind::FnDef(..)
        ) {
        ...
        if !enabled_features
            .iter()
            .any(|feature| feature == "function-contracts")
        {
            for kind in self
                .map
                .keys()
                .copied()
                .filter(|a| a.demands_function_contract_use())
            {
                let msg = format!(
        ...
        if !enabled_features.iter().any(|feature| feature == "stubbing") {
            for kind in self
                ...
                .keys()
                .copied()
                .filter(|a| a.demands_stubbing_use())
            {
                let msg = format!(
    ...
    fn report_unstable_forbidden(&self, unstable_attr: &UnstableAttribute) -> ErrorGuaranteed {
        ...
        self.tcx
            ...
            .with_help(format!(
                "use `-Z {}` to enable using this function.",
    ...
    pub fn harness_attributes(&self) -> HarnessAttributes {
        ...
        let harness_attrs = if let Some(Ok(harness)) = self.proof_for_contract() {
            HarnessAttributes::new(HarnessKind::ProofForContract {
                target_fn: harness.to_string(),
            })
        } else {
    ...
    fn check_proof_attribute(&self, kind: KaniAttributeKind, proof_attribute: &Attribute) {
        ...
        if tcx.def_kind(self.item) != DefKind::Fn {
            ...
            if !fn_abi.args.is_empty() {
                tcx.dcx().span_err(
                    span,
                    "functions used as harnesses cannot have any arguments",
                );
            }
        }
    }
    ...
    fn parse_single_path_attr(
        ...
        let current_module = self
            .tcx
            .parent_module_from_def_id(self.item.expect_local())
            .to_local_def_id();
...
/// Check if an instance is a proof harness (more efficient than full attribute parsing).
/// Performs minimal attribute lookup to check for proof or proof_for_contract attributes.
...
/// Check if a function with the given internal DefId is a proof harness.
///
/// This version works with internal rustc types directly, avoiding the need
/// for rustc_internal::stable/internal conversions that require TLV context.
pub fn is_proof_harness_from_def_id(tcx: TyCtxt, def_id: DefId) -> bool {
    has_kani_attribute(tcx, def_id, |a| {
        matches!(
            a,
            KaniAttributeKind::Proof | KaniAttributeKind::ProofForContract
...
fn expect_single<'tcx>(
    ...
    let attr = attributes.first().unwrap_or_else(|| {
        panic!(
            "expected at least one attribute {} in {attributes:?}",
            kind.as_ref()
        )
    });
    if attributes.len() > 1 {
        tcx.dcx().span_err(
            attr.span(),
            format!(
                "only one '#[kani::{}]' attribute is allowed per harness",
                kind.as_ref()
            ),
        );
    }
    ...
    fn report(&self, tcx: TyCtxt) -> ErrorGuaranteed {
        tcx.dcx()
            .struct_span_err(
                self.attr.span(),
                format!(
                    "failed to parse `#[kani::unstable_feature]`: {}",
                    self.reason
                ),
            )
            .with_note(format!(
    ...
    fn try_from(attr: &'a Attribute) -> Result<Self, Self::Error> {
        ...
        if !invalid_keys.is_empty() {
            ...
            let get_val = |name: &str| {
                args.get(name)
                    .cloned()
                    .ok_or(build_error(format!("missing `{name}` field")))
            };
...
fn expect_no_args(tcx: TyCtxt, kind: KaniAttributeKind, attr: &Attribute) {
    if !attr.is_word() {
        tcx.dcx()
            .struct_span_err(
                attr.span(),
                format!("unexpected argument for `{}`", kind.as_ref()),
            )
    ...
    match &meta_item.kind {
        MetaItemKind::Word => {
            ...
            match solver {
                ...
                Err(_) => {
                    tcx.dcx()
                        .span_err(attr.span(), format!("unknown solver `{ident_str}`"));
                    None
                }
            }
        }
...
fn parse_key_values(attr: &Attribute) -> Result<BTreeMap<String, String>, String> {
    ...
    args.iter()
        .map(|arg| match arg.meta_item() {
            Some(MetaItem {
                path: key,
                kind: MetaItemKind::NameValue(val),
                ..
            }) => Ok((
                // Safety: Paths in valid Rust syntax always have at least one segment
                key.segments.first().unwrap().ident.to_string(),
        ...
        if (!segments.is_empty()) && segments[0].as_str() == "kanitool" {
            ...
            KaniAttributeKind::try_from(ident_str.as_str())
                .inspect_err(|&err| {
                    ...
                    tcx.dcx()
                        .span_err(attr.span(), format!("unknown attribute `{ident_str}`"));
                })
                .ok()
        } else {
            None
...
fn pretty_type_path(path: &TypePath) -> String {
    ...
    let leading = if path.path.leading_colon.is_some() {
        "::"
    } else {
        ""
    };
