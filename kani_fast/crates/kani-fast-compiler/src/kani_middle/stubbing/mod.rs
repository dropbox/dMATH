...
use rustc_public::ty::{FnDef, GenericArgKind, GenericArgs, RigidTy, TyKind};
...
fn generic_args_len_without_self(args: &GenericArgs) -> usize {
    ...
    let has_self = args.0.iter().any(|arg| {
        if let Some(ty) = arg.ty() {
            matches!(ty.kind(), TyKind::Param(param_ty) if param_ty.name == "Self")
        } else {
            false
        }
    });
...
fn generic_args_without_self(args: &GenericArgs) -> impl Iterator<Item = &GenericArgKind> {
    args.0.iter().filter(|arg| {
        if let Some(ty) = arg.ty() {
            !matches!(ty.kind(), TyKind::Param(param_ty) if param_ty.name == "Self")
        } else {
            true
...
fn describe_generic_arg_kind(arg: &GenericArgKind) -> &'static str {
    match arg {
        GenericArgKind::Type(_) => "type parameter",
        GenericArgKind::Const(_) => "const parameter",
        GenericArgKind::Lifetime(_) => "lifetime parameter",
    }
}
...
pub fn check_compatibility(tcx: TyCtxt, old_def: FnDef, new_def: FnDef) -> Result<(), String> {
    ...
    let old_generic_args: Vec<_> = generic_args_without_self(&old_args).collect();
    let new_generic_args: Vec<_> = generic_args_without_self(&new_args).collect();
    let mut diff = vec![];

    for (i, (old_arg, new_arg)) in old_generic_args
        .iter()
        .zip(new_generic_args.iter())
        .enumerate()
    {
        match (old_arg, new_arg) {
            (GenericArgKind::Type(old_ty), GenericArgKind::Type(new_ty)) => {
                if old_ty != new_ty {
                    diff.push(format!(
                        "Expected generic type parameter {} to be `{old_ty}`, but found `{new_ty}`",
                        i + 1
                    ));
                }
            }
            (GenericArgKind::Const(old_const), GenericArgKind::Const(new_const)) => {
                if old_const != new_const {
                    diff.push(format!(
                        "Expected generic const parameter {} to be `{old_const:?}`, but found `{new_const:?}`",
                        i + 1
                    ));
                }
            }
            (GenericArgKind::Lifetime(old_lt), GenericArgKind::Lifetime(new_lt)) => {
                if old_lt != new_lt {
                    diff.push(format!(
                        "Expected generic lifetime parameter {} to be `{old_lt:?}`, but found `{new_lt:?}`",
                        i + 1
                    ));
                }
            }
            _ => diff.push(format!(
                "Generic parameter {} kind mismatch: expected {}, but found {}",
                i + 1,
                describe_generic_arg_kind(old_arg),
                describe_generic_arg_kind(new_arg),
            )),
        }
    }
    ...
    match (old_def.body(), new_def.body()) {
        (Some(old_body), Some(new_body)) => {
            ...
            if old_ret_ty != new_ret_ty {
                diff.push(format!(
                    "Expected return type `{old_ret_ty}`, but found `{new_ret_ty}`"
                ));
            }
            for (i, (old_arg, new_arg)) in old_body
                .arg_locals()
                .iter()
        ...
        // If either side is missing a body, fall back to checking the signatures directly.
        _ => {
            ...
            if old_sig.output() != new_sig.output() {
                diff.push(format!(
            ...
            for (i, (old_arg, new_arg)) in old_sig
                .inputs()
                .iter()
...
impl MirVisitor for StubConstChecker<'_> {
    ...
    fn visit_const_operand(&mut self, constant: &ConstOperand, location: Location) {
        ...
        generic bounds are not being met.",
                    ...
                    tcx.dcx()
                        .span_err(rustc_internal::internal(self.tcx, location.span()), msg);
                    self.is_valid = false;
                }
            }
        };
