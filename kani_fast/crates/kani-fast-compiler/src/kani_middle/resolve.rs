...
fn resolve_path<'tcx>(
    ...
    path.segments
        .into_iter()
        .try_fold(
            (path.base, path.base_path_args.clone()),
            |(base, base_path_args), segment| {
...
fn resolve_prefix<'tcx>(
    ...
    match (path.leading_colon, segments.next()) {
        ...
        (Some(_), None) => Err(ResolveError::InvalidPath {
...
fn resolve_in_foreign_module(tcx: TyCtxt, foreign_mod: DefId, name: &str) -> Option<DefId> {
    ...
    tcx.module_children(foreign_mod).iter().find_map(|item| {
...
fn resolve_in_any_trait<'tcx>(tcx: TyCtxt<'tcx>, ty: Ty, name: &str) -> Option<FnResolution> {
    let matches: Vec<FnResolution> = rustc_public::all_trait_decls()
        ...
        .filter_map(|trait_def| {
            resolve_in_trait_def_stable(tcx, trait_def, name)
                .ok()
                .and_then(|item| {
...
fn resolve_in_trait_impl(
    ...
    exists.map_or(
        Err(ResolveError::MissingTraitImpl {
            tcx,
            trait_fn_id,
            ty,
        }),
        |inst| {
            Ok(FnResolution::FnImpl {
                def: inst.ty().kind().fn_def().unwrap().0,
                ty,
            })
        },
    )
...
fn resolve_relative(tcx: TyCtxt, current_module: LocalModDefId, name: &str) -> RelativeResolution {
    ...
    let result = tcx
        .hir_module_free_items(current_module)
        .find_map(|item_id| {
            ...
            {
                match item.kind {
                    ItemKind::Use(use_path, UseKind::Single(_)) => use_path
                        .res
                        .present_items()
    ...
    I: Iterator<Item = &'a PathSegment>,
{
    let def_id = match ty.kind() {
        ...
        TyKind::Param(_) | TyKind::Bound(_, _) => {
            // Name resolution cannot resolve in a parameter or bound.
            unreachable!()
        }
    };
...
fn resolve_in_trait_def_stable<'tcx>(
    ...
    let missing_item_err = || ResolveError::MissingItem {
        tcx,
        base: trait_def_def_id,
        unresolved: name.to_string(),
    };
    ...
    I: Iterator<Item = &'a PathSegment>,
{
    if let Some(next) = segments.next() {
        ...
        Err(ResolveError::InvalidPath {
            msg: format!("Unexpected primitive type `{ty}`"),
        })
    }
}
...
fn is_item_name(tcx: TyCtxt, item: DefId, name: &str) -> bool {
    ...
    // Safety: split() always yields at least one element, even for empty strings
