...
impl CodegenUnits {
    pub fn new(queries: &QueryDb, tcx: TyCtxt) -> Self {
        let crate_info = CrateInfo {
            name: rustc_public::local_crate().name.as_str().into(),
        };
        ...
        match args.reachability_analysis {
            ...
            ReachabilityType::AllFns => {
                ...
                let kani_harness_intrinsic = kani_fns
    ...
    pub fn is_automatic_harness(&self, harness: &Harness) -> bool {
        self.harness_info
            .get(harness)
            .is_some_and(|md| md.is_automatically_generated)
    }
    ...
    pub fn harness_model_path(&self, harness: Harness) -> &PathBuf {
        &self.harness_info[&harness].goto_file
    }
    ...
    fn generate_metadata(&self, tcx: TyCtxt) -> KaniMetadata {
        let (proof_harnesses, test_harnesses) = self
            ...
            .values()
            .cloned()
            .partition(|md| md.attributes.is_proof_harness());
        KaniMetadata {
...
fn stub_def(tcx: TyCtxt, def_id: DefId) -> FnDef {
    ...
    if let TyKind::RigidTy(RigidTy::FnDef(def, _)) = ty.kind() {
        ...
        unreachable!(
            "Expected stub function for `{:?}`, but found: {ty}",
            tcx.def_path(def_id)
        )
    }
}
...
fn metadata_output_path(tcx: TyCtxt) -> PathBuf {
    ...
    filename
        .with_extension(ArtifactType::Metadata)
        .to_path_buf()
}
...
fn validate_units(tcx: TyCtxt, units: &[CodegenUnit]) {
    for unit in units {
        for (from, to) in &unit.stubs {
            ...
            let Err(msg) = check_compatibility(tcx, *from, *to) else {
                continue;
            };
            // Safety: CodegenUnit is always created with at least one harness
...
fn automatic_harness_partition(
    ...
    let crate_fn_defs = rustc_public::local_crate()
        .fn_defs()
        .into_iter()
        .collect::<FxHashSet<_>>();
        ...
        for (idx, arg) in body.arg_locals().iter().enumerate() {
            ...
            if !impls_arbitrary {
                ...
                let arg_name = body
                    ...
                    .find(|var| {
                        var.argument_index
                            .is_some_and(|arg_idx| idx + 1 == usize::from(arg_idx))
                    })
                    .map_or("_".to_string(), |debug_info| debug_info.name.to_string());
                let arg_type = arg.ty.to_string();
                problematic_args.push((arg_name, arg_type))
            }
        }
        if !problematic_args.is_empty() {
            return Some(AutoHarnessSkipReason::MissingArbitraryImpl(
                problematic_args,
            ));
        }
        None
...
mod autoharness_filter_tests {
    ...
    fn exclude_specific_module() {
        ...
        assert!(autoharness_filtered_out(
            "crate::internal::helper_fn",
            &included,
            &excluded
        ));
        assert!(autoharness_filtered_out(
            "my_crate::internal::test_fn",
            &included,
            &excluded
        ));
        assert!(!autoharness_filtered_out(
            "crate::public::test_fn",
            &included,
            &excluded
        ));
    }
    ...
    fn test_exact_match_with_crate() {
        ...
        assert!(autoharness_filtered_out(
            "lib::foo_function_extra",
            &included,
            &excluded
        ));
        assert!(autoharness_filtered_out(
            "lib::other::foo_function",
            &included,
            &excluded
        ));
        assert!(autoharness_filtered_out(
    ...
    fn complex_path_patterns() {
        ...
        let excluded = make_regex_set(vec![
            ".*::internal::.*".to_string(),
            ".*::private::.*".to_string(),
        ]);
