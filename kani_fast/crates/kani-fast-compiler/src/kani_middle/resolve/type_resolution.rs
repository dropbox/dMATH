...
pub fn resolve_ty<'tcx>(
    ...
    let invalid = |kind: &'static str| {
        Err(ResolveError::InvalidPath {
            msg: format!(
                "Expected a type, but found {kind} `{}`",
                typ.to_token_stream()
            ),
        })
    };
    ...
    match typ {
        Type::Path(TypePath { qself, path }) => {
            ...
            if let Some(primitive) = path
                .get_ident()
                .and_then(|ident| PrimitiveIdent::from_str(&ident.to_string()).ok())
            {
                Ok(primitive.into())
            } else {
        ...
        Type::Array(array) => {
            ...
            Ty::try_new_array(elem_ty, len.try_into().unwrap()).map_err(|err| {
                ResolveError::InvalidPath {
                    msg: format!("Cannot instantiate array. {err}"),
                }
            })
        }
    ...
    T: ToTokens,
{
    ...
    let Ok(typ) = syn::parse2(token) else {
        return false;
    };
...
fn parse_len(len: &Expr) -> Result<usize, String> {
    ...
    Err(format!(
        "Expected a `usize` constant, but found `{}`",
        len.to_token_stream()
    ))
}
