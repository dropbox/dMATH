...
pub fn gen_contracts_metadata(
    ...
    for (harness, metadata) in harness_info {
        ...
        if let HarnessKind::ProofForContract { target_fn } = &metadata.attributes.kind {
            ...
            if let Some((_, target_cf)) = fn_to_data
                .iter_mut()
                .find(|(_, cf)| &cf.function == target_fn)
            {
                ...
                warn!(
                    ...
                    "automatic contract harness target missing function; skipping"
                );
            }
        }
    }
...
/// the metadata from that function uniquely identifies the harness.
