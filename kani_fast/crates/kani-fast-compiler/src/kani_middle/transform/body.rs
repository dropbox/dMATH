...
impl MutableBody {
    ...
    pub fn var_debug_info(&self) -> &[VarDebugInfo] {
        &self.var_debug_info
    }
    ...
    pub fn insert_check(
        ...
        let (assert_fn, args) = match check_type {
            CheckType::SafetyCheck(assert_fn) | CheckType::SafetyCheckNoAssume(assert_fn) => {
                ...
                (
                    assert_fn,
                    vec![Operand::Move(Place::from(value.unwrap())), msg_op],
                )
            }
    ...
    pub fn insert_call(
        ...
        let new_bb = self.blocks.len();
        let span = source.span(&self.blocks);
        let callee_op = Operand::Copy(Place::from(self.new_local(
            callee.ty(),
            span,
            Mutability::Not,
        )));
            ...
            InsertPosition::After => {
                ...
                match source {
                    ...
                    SourceInstruction::Terminator { bb } => {
                        ...
                        *source = SourceInstruction::Statement {
                            idx: 0,
                            bb: new_bb_idx,
                        };
                        let new_bb = BasicBlock {
