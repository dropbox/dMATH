...
impl TransformPass for IntrinsicGeneratorPass {
    ...
    fn transform(&mut self, tcx: TyCtxt, body: Body, instance: Instance) -> (bool, Body) {
        ...
        if let Some(kani_intrinsic) = attributes
            .fn_marker()
            .and_then(|name| KaniIntrinsic::from_str(name.as_str()).ok())
        {
            match kani_intrinsic {
                ...
                // The former two are handled in contracts pass for now, while the latter is handled in the automatic harness pass.
...
impl IntrinsicGeneratorPass {
    ...
    fn valid_value_body(&self, body: Body) -> Body {
        ...
        let TyKind::RigidTy(RigidTy::RawPtr(target_ty, _)) = arg_ty.kind() else {
            unreachable!()
        };
    ...
    fn is_initialized_body(&mut self, body: Body) -> Body {
        ...
        let TyKind::RigidTy(RigidTy::RawPtr(pointee_ty, _)) = arg_ty.kind() else {
            unreachable!()
        };
                ...
                match pointee_info.layout() {
                    PointeeLayout::Sized { layout } => {
                        ...
                        let is_ptr_initialized_instance = resolve_mem_init_fn(
                            *self
                                .kani_defs
                                .get(&KaniModel::IsPtrInitialized.into())
                                .unwrap(),
                            layout.len(),
                            *pointee_info.ty(),
                        );
                    ...
                    PointeeLayout::Slice { element_layout } => {
                        ...
                        let (slicee_ty, intrinsic) = match pointee_info.ty().kind() {
                            ...
                            TyKind::RigidTy(RigidTy::Str) => (
                                ...
                                KaniModel::IsStrPtrInitialized.into(),
                            ),
                            _ => unreachable!(),
                        };
                        let is_ptr_initialized_instance = resolve_mem_init_fn(
    ...
    fn checked_size_of(&mut self, body: Body, instance: Instance) -> Body {
        ...
        let ptr_arg = body
            .arg_locals()
            .first()
            .expect("Expected a pointer argument");
...
fn build_some(option: AdtDef, args: GenericArgs, val_op: Operand) -> Rvalue {
    let var_idx = option
        ...
        .unwrap();
    Rvalue::Aggregate(
        AggregateKind::Adt(option, var_idx, args, None, None),
        vec![val_op],
    )
}
