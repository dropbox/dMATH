...
impl LoopContractPass {
    pub fn new(_tcx: TyCtxt, queries: &QueryDb, unit: &CodegenUnit) -> LoopContractPass {
        if !unit.harnesses.is_empty() {
            let run_contract_fn = queries
                .kani_functions()
                .get(&KaniModel::RunLoopContract.into())
                .copied();
            assert!(
                run_contract_fn.is_some(),
                "Failed to find Kani run contract function"
            );
            LoopContractPass {
                run_contract_fn,
    ...
    fn replace_first_pat_by_nth_pat(&self, body: &mut MutableBody) {
        ...
        for (firstvar, nthvar, first_blockid, nth_blockid) in first_nth_list {
            ...
            body.insert_stmt(
                storagelive_stmt,
                &mut SourceInstruction::Statement {
                    idx: 0,
                    bb: first_blockid,
                },
    ...
    fn insert_blocks_from_loophead(body: &mut MutableBody, blocks: &[BasicBlock], loophead: usize) {
        for (i, block) in blocks.iter().enumerate() {
            if i == 0 {
                ...
                body.insert_bb(
                    modified_block,
                    &mut SourceInstruction::Terminator {
                        bb: body.blocks().len() - 1,
                    },
    ...
    fn move_storagedead(&self, body: &mut MutableBody, src_block_idx: usize, dst_block_idx: usize) {
        ...
        let other_stmts: Vec<_> = body.blocks()[src_block_idx]
            ...
            .collect();
        body.replace_statements(
            &SourceInstruction::Terminator { bb: src_block_idx },
            other_stmts,
        );
        ...
        body.replace_statements(
            &SourceInstruction::Terminator { bb: dst_block_idx },
            new_stmts,
        );
    }
    ...
    fn transform_bb(&mut self, tcx: TyCtxt, new_body: &mut MutableBody, bb_idx: usize) -> bool {
        ...
        } = &terminator.kind
        {
                ...
                && matches!(&terminator_args[1], Operand::Constant(op) if op.const_.eval_target_usize().unwrap() == 0)
            {
                ...
                if !new_body.blocks()[terminator_target.unwrap()]
                    .statements
                    .is_empty()
                    || !matches!(
                        new_body.blocks()[terminator_target.unwrap()]
                            .terminator
                            .kind,
                ...
                let GenericArgKind::Type(arg_ty) = genarg.0[0] else {
                    return false;
                };
                ...
                let GenericArgKind::Type(arg_ty) = genarg.0[fnptrpos + 1] else {
                    return false;
                };
                let TyKind::RigidTy(RigidTy::Tuple(args)) = arg_ty.kind() else {
                    return false;
                };
                // Check if the invariant involves any local variable
                if !args.is_empty() {
                    let ori_condition_bb_idx = new_body.blocks()[terminator_target.unwrap()]
                        .terminator
                        .successors()[1];
                ...
                self.new_loop_latches
                    .insert(bb_idx, new_body.blocks().len() - 1);
            }
        }
