...
impl RustcInternalMir for ConstOperand {
    ...
    fn internal_mir<'tcx>(&self, tcx: TyCtxt<'tcx>) -> Self::T<'tcx> {
        rustc_middle::mir::ConstOperand {
            ...
            user_ty: self
                .user_ty
                .map(rustc_ty::UserTypeAnnotationIndex::from_usize),
...
impl RustcInternalMir for BorrowKind {
    ...
    fn internal_mir<'tcx>(&self, tcx: TyCtxt<'tcx>) -> Self::T<'tcx> {
        match self {
            ...
            BorrowKind::Mut { kind } => rustc_middle::mir::BorrowKind::Mut {
                kind: kind.internal_mir(tcx),
            },
        }
    }
}
        ...
        match self {
            ...
            Rvalue::Aggregate(aggregate_kind, operands) => rustc_middle::mir::Rvalue::Aggregate(
                ...
                rustc_index::IndexVec::from_raw(
                    operands
                        .iter()
                        .map(|operand| operand.internal_mir(tcx))
                        .collect(),
                ),
            ),
    ...
    fn internal_mir<'tcx>(&self, tcx: TyCtxt<'tcx>) -> Self::T<'tcx> {
        match self {
            ...
            TerminatorKind::Call {
                ...
                args: Box::from_iter(
                    args.iter()
                        .map(|arg| rustc_span::source_map::dummy_spanned(arg.internal_mir(tcx))),
                ),
...
impl RustcInternalMir for Body {
    ...
    fn internal_mir<'tcx>(&self, tcx: TyCtxt<'tcx>) -> Self::T<'tcx> {
        ...
        let local_decls = rustc_index::IndexVec::from_raw(
            self.locals()
                .iter()
                .map(|local_decl| local_decl.internal_mir(tcx))
                .collect(),
        );
