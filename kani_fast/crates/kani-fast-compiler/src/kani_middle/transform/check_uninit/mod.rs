...
impl<'a> UninitInstrumenter<'a> {
    ...
    fn instrument(
        ...
        if KaniFunction::try_from(instance).is_ok_and(|f| SKIPPED_ITEMS.contains(&f)) {
            return (false, body);
        }
    ...
    fn build_get_and_check(
        ...
        let terminator = match pointee_info.layout() {
            PointeeLayout::Sized { layout } => {
                ...
                // Depending on whether we are accessing the known number of elements in the slice,
            ...
            PointeeLayout::Slice { element_layout } => {
                // Since `str` is a separate type, we need to differentiate between [T] and str.
    ...
    fn build_set(
        ...
        let terminator = match pointee_info.layout() {
            PointeeLayout::Sized { layout } => {
                let layout_operand = mk_layout_operand(body, &mut statements, source, layout);
                // Depending on whether we are writing to the known number of elements in the slice,
                // we need to pass it as an argument.
                let (diagnostic, args) = match &operation {
                ...
                Terminator {
                    kind: TerminatorKind::Call {
                        ...
                        args,
                        destination: ret_place.clone(),
                        target: Some(0), // this will be overridden in add_bb
                        unwind: UnwindAction::Terminate,
                    },
                    span: source.span(body.blocks()),
                }
