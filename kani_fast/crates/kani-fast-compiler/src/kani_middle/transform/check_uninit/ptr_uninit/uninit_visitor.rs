...
impl MirVisitor for CheckUninitVisitor {
        ...
        match &stmt.kind {
            ...
            StatementKind::Assign(place, rvalue) => {
                ...
                if let Some(place_without_deref) = try_remove_topmost_deref(place) {
                    ...
                    let mut place_to_add_projections = Place {
                        local: place_without_deref.local,
                        projection: vec![],
                    };
                    for projection_elem in place_without_deref.projection.iter() {
                        ...
                        place_to_add_projections
                            .projection
                            .push(projection_elem.clone());
                    }
    ...
    fn visit_place(&mut self, place: &Place, ptx: PlaceContext, location: Location) {
        for (idx, elem) in place.projection.iter().enumerate() {
            let intermediate_place = Place {
                local: place.local,
                projection: place.projection[..idx].to_vec(),
            };
            match elem {
