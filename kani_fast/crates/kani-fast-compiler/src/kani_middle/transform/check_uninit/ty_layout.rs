    ...
    pub fn maybe_size(&self) -> Option<usize> {
        match self {
            ...
            PointeeLayout::Union { field_layouts } => Some(
                field_layouts
                    .iter()
                    .map(|field_layout| field_layout.len())
                    .max()
                    .unwrap(),
            ),
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            LayoutComputationError::EnumWithNicheEncoding(ty) => {
                write!(
                    f,
                    "Cannot determine layout for an Enum with niche encoding of type {ty}"
                )
            }
    ...
    pub fn from_ty(ty: Ty) -> Result<Self, LayoutComputationError> {
        match ty.kind() {
            TyKind::RigidTy(rigid_ty) => match rigid_ty {
                ...
                RigidTy::Dynamic(..) => Ok(PointeeInfo {
                    pointee_ty: ty,
                    layout: PointeeLayout::TraitObject,
                }),
                _ => {
...
fn data_bytes_for_ty(
    ...
    match layout.fields {
        FieldsShape::Primitive => Ok(vec![match layout.abi {
            ValueAbi::Scalar(Scalar::Initialized { value, .. }) => DataBytes {
        ...
        FieldsShape::Arbitrary { ref offsets } => {
            match ty
                .kind()
                .rigid()
                .unwrap_or_else(|| panic!("unexpected type: {ty:?}"))
            {
                        ...
                        AdtKind::Enum => {
                            ...
                            match layout.variants {
                                ...
                                VariantsShape::Multiple { variants, tag, .. } => {
                                    ...
                                    let tag_data_bytes = vec![DataBytes {
                                        offset: current_offset,
                                        size: tag_size,
                                    }];
