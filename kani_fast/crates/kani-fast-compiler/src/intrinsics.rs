...
impl Intrinsic {
    ...
    pub fn from_instance(intrinsic_instance: &Instance) -> Self {
        // Safety: This function is only called on instances that are known to be intrinsics,
        // so intrinsic_name() is guaranteed to return Some.
        let intrinsic_str = intrinsic_instance
            .intrinsic_name()
            .expect("from_instance called on non-intrinsic");
        // Safety: Intrinsics are always functions with a valid function signature.
        let sig = intrinsic_instance
            .ty()
            .kind()
            .fn_sig()
            .expect("intrinsic has no function signature")
            .skip_binder();
...
fn try_match_atomic(intrinsic_instance: &Instance) -> Option<Intrinsic> {
    ...
    let intrinsic_str = intrinsic_instance
        .intrinsic_name()
        .expect("try_match_atomic called on non-intrinsic");
    // Safety: Intrinsics are always functions with a valid function signature.
    let sig = intrinsic_instance
        .ty()
        .kind()
...
fn try_match_simd(intrinsic_instance: &Instance) -> Option<Intrinsic> {
    ...
    let intrinsic_str = intrinsic_instance
        .intrinsic_name()
        .expect("try_match_simd called on non-intrinsic");
    // Safety: Intrinsics are always functions with a valid function signature.
    let sig = intrinsic_instance
        .ty()
        .kind()
...
fn try_match_f32(intrinsic_instance: &Instance) -> Option<Intrinsic> {
    ...
    let intrinsic_str = intrinsic_instance
        .intrinsic_name()
        .expect("try_match_f32 called on non-intrinsic");
    // Safety: Intrinsics are always functions with a valid function signature.
    let sig = intrinsic_instance
        .ty()
        .kind()
...
fn try_match_f64(intrinsic_instance: &Instance) -> Option<Intrinsic> {
    ...
    let intrinsic_str = intrinsic_instance
        .intrinsic_name()
        .expect("try_match_f64 called on non-intrinsic");
    // Safety: Intrinsics are always functions with a valid function signature.
    let sig = intrinsic_instance
        .ty()
        .kind()
