...
Kani Fast will provide test proofs:

1. `overflow_proof.lean` - Prove no overflow in checked_add
2. `bounds_proof.lean` - Prove array access in bounds
3. `loop_invariant.lean` - Prove loop maintains invariant
4. `impossible.lean` - Should FAIL (tests error reporting)

---

## Current Lean4 Usage

We currently use Lean4 via:
```bash
lean --version
lean4 file.lean
```

Lean5 should be command-compatible or document differences.
...
- **Proof generation code:** `crates/kani-fast-lean5/` (stub, will implement)
