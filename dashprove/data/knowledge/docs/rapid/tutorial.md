...
Here is what a trivial test using rapid looks like ([playground][3]):
...
* `ParseDate` function test: [source code][4], [playground][5]
* `Queue` state machine test: [source code][6], [playground][7]

## Comparison

Rapid aims to bring to Go the power and convenience [Hypothesis][8] brings to Python.

Compared to [testing.F.Fuzz][9], rapid shines in generating complex structured data, including state
machine tests, but lacks coverage-guided feedback and mutations. Note that with [`MakeFuzz`][10],
any rapid test can be used as a fuzz target for the standard fuzzer.

Compared to [gopter][11], rapid provides a much simpler API (queue test in [rapid][12] vs
[gopter][13]), is much smarter about data generation and is able to minimize failing test cases
fully automatically, without any user code.

As for [testing/quick][14], it lacks both convenient data generation facilities and any form of test
...
[`MakeFuzz`][15], any rapid test can be used as a fuzz target.
...
Rapid is licensed under the [Mozilla Public License Version 2.0][16].

[1]: https://pkg.go.dev/pgregory.net/rapid
[2]: https://github.com/flyingmutant/rapid/actions
[3]: https://go.dev/play/p/QJhOzo_BByz
[4]: /flyingmutant/rapid/blob/master/example_function_test.go
[5]: https://go.dev/play/p/tZFU8zv8AUl
[6]: /flyingmutant/rapid/blob/master/example_statemachine_test.go
[7]: https://go.dev/play/p/cxEh4deG-4n
[8]: https://github.com/HypothesisWorks/hypothesis
[9]: https://pkg.go.dev/testing#F.Fuzz
[10]: https://pkg.go.dev/pgregory.net/rapid#MakeFuzz
[11]: https://pkg.go.dev/github.com/leanovate/gopter
[12]: /flyingmutant/rapid/blob/master/example_statemachine_test.go
[13]: https://github.com/leanovate/gopter/blob/90cc76d7f1b21637b4b912a7c19dea3efe145bb2/commands/exa
mple_circularqueue_test.go
[14]: https://pkg.go.dev/testing/quick
[15]: https://pkg.go.dev/pgregory.net/rapid#MakeFuzz
[16]: /flyingmutant/rapid/blob/master/LICENSE
