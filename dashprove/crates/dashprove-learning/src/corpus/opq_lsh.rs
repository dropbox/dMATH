//! Hybrid OPQ-LSH index for memory-efficient approximate search with reduced quantization error
//!
//! This module combines Optimized Product Quantization (OPQ) with Locality-Sensitive Hashing (LSH)
//! to provide memory-efficient approximate nearest neighbor search with lower quantization error
//! than standard PQ-LSH:
//!
//! - **LSH for candidate filtering**: O(1) lookup to find candidate buckets
//! - **OPQ for compact storage**: 48x compression with 10-30% lower quantization error than PQ
//! - **OPQ distance for reranking**: Fast approximate distance using precomputed tables
//!
//! # Architecture
//!
//! ```text
...
//!       v
//! +-------------+
//! | OPQ Rerank  | -> Score candidates using OPQ distance table (O(M) per candidate)
//! +-------------+
//!       |
//!       v
//!    Top-k results
...
//! |--------|--------|----------|
//! | Exact search | 38.4 MB | Exact |
//! | PQ-LSH | 0.8 MB + codebooks + hash | ~baseline |
//! | **OPQ-LSH** | **0.8 MB + codebooks + rotation + hash** | **10-30% better** |
//!
//! OPQ-LSH provides similar memory efficiency to PQ-LSH but with lower quantization error
//! due to the learned rotation matrix that decorrelates dimensions within subspaces.

use super::types::ProofId;
use crate::embedder::Embedding;
use crate::lsh::{LshConfig, LshIndex};
use crate::pq::{OpqConfig, OptimizedProductQuantizer, PqError};
use crate::similarity::SimilarProof;
use serde::{Deserialize, Serialize};
use std::collections::HashMap;

/// Configuration for hybrid OPQ-LSH index
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct OpqLshConfig {
    /// LSH configuration for candidate filtering
    pub lsh: LshConfig,
    /// OPQ configuration for compact storage with rotation
    pub opq: OpqConfig,
}

impl Default for OpqLshConfig {
    fn default() -> Self {
        Self {
...
impl OpqLshConfig {
    ...
    pub fn for_large_corpus() -> Self {
        Self {
            ...
            opq: OpqConfig::default(), // Higher accuracy for large corpus
        }
    }
...
/// Hybrid OPQ-LSH index for ProofCorpus
///
/// Uses LSH for fast candidate filtering and OPQ for memory-efficient
/// storage and distance computation with reduced quantization error.
#[derive(Debug)]
pub struct ProofCorpusOpqLsh {
    /// LSH index for candidate generation
    lsh: LshIndex,
    /// Optimized product quantizer for encoding embeddings
    quantizer: OptimizedProductQuantizer,
    /// OPQ codes for each entry: entries[i] = (proof_id, opq_codes)
...
impl ProofCorpusOpqLsh {
    /// Build an OPQ-LSH index from a ProofCorpus
    ///
    /// Requires at least `opq.pq.num_centroids` embeddings for training.
    ...
    pub fn build(corpus: &super::storage::ProofCorpus) -> Result<Option<Self>, PqError> {
        Self::build_with_config(corpus, OpqLshConfig::default())
    }
    ...
    pub fn build_with_config(
        ...
        if training_data.len() < config.opq.pq.num_centroids {
        ...
        let quantizer = OptimizedProductQuantizer::train(&training_data, config.opq.clone())?;
        ...
        for entry in corpus.entries() {
            if let Some(ref embedding) = entry.embedding {
                ...
                // Store OPQ codes
                let opq_codes = quantizer.encode(embedding);
                entries.push((entry.id.clone(), opq_codes));
    ...
    pub fn build_auto_config(
        ...
        let config = if count < 1000 {
            OpqLshConfig::for_small_corpus()
        } else if count < 10000 {
            OpqLshConfig::for_medium_corpus()
        } else if count < 100000 {
            OpqLshConfig::for_large_corpus()
        } else {
            OpqLshConfig::for_huge_corpus()
        };
    ...
    pub fn memory_stats(&self) -> OpqLshMemoryStats {
        let num_entries = self.entries.len();
        let num_subspaces = self.config.opq.pq.num_subspaces;

        // OPQ codes memory: entries * subspaces * 1 byte
        let opq_codes_bytes = num_entries * num_subspaces;

        // Codebook + rotation matrix memory
        let codebook_bytes = self.quantizer.total_size_bytes();
        ...
        OpqLshMemoryStats {
            ...
            codebook_bytes,
            lsh_bytes,
            total_bytes: opq_codes_bytes + codebook_bytes + lsh_bytes,
            raw_equivalent_bytes: raw_bytes,
            compression_ratio: if opq_codes_bytes + codebook_bytes > 0 {
                raw_bytes as f64 / (opq_codes_bytes + codebook_bytes) as f64
            } else {
                0.0
            },
        }
    ...
    /// OPQ asymmetric distance computation.
    pub fn find_similar_approximate(
        ...
        // Build OPQ distance table for fast reranking
        let distance_table = self.quantizer.build_distance_table(query);

        // Rerank candidates using OPQ distance
        let mut scored: Vec<(usize, f64)> = lsh_results
            ...
            .filter_map(|(lsh_id, _)| {
                ...
                let sim = OptimizedProductQuantizer::distance_to_similarity(dist);
                Some((idx, sim))
            })
            .collect();
    ...
    /// Exact search using OPQ (bypasses LSH, examines all entries)
    ///
    /// Useful for comparing recall quality.
    pub fn find_similar_exact_opq(
        &self,
        corpus: &super::storage::ProofCorpus,
        query: &Embedding,
        k: usize,
    ...
    /// Measure recall of OPQ-LSH vs exact OPQ search
    ///
    ...
    pub fn measure_recall(
        ...
        for i in (0..entries.len()).step_by(step) {
            ...
            if let Some(ref query) = entry.embedding {
                ...
                let exact = self.find_similar_exact_opq(corpus, query, k);
    ...
    pub fn config(&self) -> &OpqLshConfig {
        &self.config
    }
    ...
    /// Save the trained OPQ quantizer to a file
    ///
    /// The quantizer contains the trained codebooks and rotation matrix which are
    /// expensive to compute. Saving allows reusing the same quantizer without retraining.
    ...
    /// Build an OPQ-LSH index using a pre-trained quantizer
    ///
    /// This skips the expensive k-means and rotation training step by reusing an existing
    /// quantizer. The quantizer must have been trained on similar data for good results.
    pub fn build_with_quantizer(
        ...
        for entry in corpus.entries() {
            if let Some(ref embedding) = entry.embedding {
                ...
                // Store OPQ codes using pre-trained quantizer
                let opq_codes = quantizer.encode(embedding);
                entries.push((entry.id.clone(), opq_codes));
                id_to_idx.insert(lsh_id, idx);
            }
    ...
    /// Build an OPQ-LSH index, loading quantizer from file if available
    ///
    ...
    pub fn build_with_cached_quantizer<P: AsRef<std::path::Path>>(
        ...
        if let Some(quantizer) = OptimizedProductQuantizer::load_if_exists(path)? {
    ...
    /// Get quantization error compared to PQ
    ///
    /// Returns the mean squared reconstruction error of OPQ on the indexed embeddings.
    /// Lower is better. Compare with PqCorpusPqLsh to see improvement from rotation.
    pub fn quantization_error(&self, corpus: &super::storage::ProofCorpus) -> f32 {
        let embeddings: Vec<_> = corpus
            .entries()
            .filter_map(|e| e.embedding.clone())
            .collect();
        self.quantizer.quantization_error(&embeddings)
    }
}

/// Memory usage statistics for OPQ-LSH index
#[derive(Debug, Clone)]
pub struct OpqLshMemoryStats {
    /// Number of indexed entries
    pub num_entries: usize,
    /// Bytes used for OPQ codes
    pub opq_codes_bytes: usize,
    /// Bytes used for OPQ codebooks + rotation matrix
...
mod tests {
    ...
    #[test]
    fn test_opq_lsh_config_default() {
        let config = OpqLshConfig::default();
        assert_eq!(config.lsh.num_tables, 8);
        assert_eq!(config.opq.pq.num_centroids, 64); // fast config
    }

    #[test]
    fn test_opq_lsh_config_presets() {
        let small = OpqLshConfig::for_small_corpus();
        let large = OpqLshConfig::for_large_corpus();
        let huge = OpqLshConfig::for_huge_corpus();
    ...
    fn test_build_empty_corpus() {
        ...
        let result = ProofCorpusOpqLsh::build(&corpus);
        assert!(result.is_ok());
    ...
    fn test_build_insufficient_embeddings() {
        ...
        // OpqConfig::fast requires 64 centroids, so 10 embeddings should fail
        let result = ProofCorpusOpqLsh::build(&corpus);
    ...
    fn test_build_with_sufficient_embeddings() {
        ...
        let index = ProofCorpusOpqLsh::build(&corpus).unwrap();
    ...
    fn test_find_similar_approximate() {
        ...
        let index = ProofCorpusOpqLsh::build(&corpus).unwrap().unwrap();
    ...
    #[test]
    fn test_find_similar_exact_opq() {
        let mut corpus = super::super::storage::ProofCorpus::new();
        ...
        let results = index.find_similar_exact_opq(&corpus, &query, 5);
    ...
    fn test_memory_stats() {
        ...
        // Compression ratio (codes only vs raw) should indicate compression
        assert!(stats.compression_ratio > 1.0);
        // OPQ codes themselves are much smaller than raw embeddings
        assert!(stats.opq_codes_bytes < stats.raw_equivalent_bytes);
    }
    ...
    fn test_auto_config() {
        ...
        let index = ProofCorpusOpqLsh::build_auto_config(&corpus).unwrap();
        assert!(index.is_some());
        let index = index.unwrap();
    ...
    fn test_save_load_quantizer() {
        ...
        let path = dir.path().join("opq_quantizer.json");
        index.save_quantizer(&path).unwrap();
        assert!(path.exists());

        // Load and rebuild with saved quantizer
        let loaded_quantizer = crate::pq::OptimizedProductQuantizer::load_from_file(&path).unwrap();
        let config = OpqLshConfig::default();
        let rebuilt = ProofCorpusOpqLsh::build_with_quantizer(&corpus, loaded_quantizer, config);
    ...
    fn test_build_with_cached_quantizer() {
        ...
        let dir = tempfile::tempdir().unwrap();
        let path = dir.path().join("cached_opq_quantizer.json");

        // First build - should train and save quantizer
        assert!(!path.exists());
        let config = OpqLshConfig::default();
        let index1 = ProofCorpusOpqLsh::build_with_cached_quantizer(&corpus, &path, config.clone())
            .unwrap()
            .unwrap();
        assert!(path.exists()); // Quantizer was saved

        // Second build - should load existing quantizer
        let index2 = ProofCorpusOpqLsh::build_with_cached_quantizer(&corpus, &path, config)
            .unwrap()
            .unwrap();
    ...
    fn test_get_quantizer() {
        ...
        assert_eq!(codes.len(), quantizer.pq_config().num_subspaces);
    }

    #[test]
    fn test_opq_lsh_config_serialize() {
        let config = OpqLshConfig::for_large_corpus();
        ...
        // Deserialize back
        let loaded: OpqLshConfig = serde_json::from_str(&json).unwrap();
        assert_eq!(loaded.lsh.num_tables, config.lsh.num_tables);
        assert_eq!(loaded.opq.pq.num_centroids, config.opq.pq.num_centroids);
    }
    ...
    fn test_quantization_error() {
        ...
        let index = ProofCorpusOpqLsh::build(&corpus).unwrap().unwrap();
        let error = index.quantization_error(&corpus);

        // Error should be non-negative
        assert!(error >= 0.0);
        // For random data with OPQ, error should be reasonably bounded
        assert!(error < 10.0); // Sanity check
    }
}
