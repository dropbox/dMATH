//! HNSW-PQ hybrid index for memory-efficient graph-based search
//!
//! This module combines HNSW's graph-based navigation with Product Quantization's
//! memory-efficient storage to provide:
//!
//! - **O(log N) search time**: Hierarchical graph enables fast navigation
//! - **~48x memory compression**: PQ compresses embeddings from 384 to 8 bytes
//! - **High recall**: Graph connectivity with asymmetric distance preserves quality
//!
//! # Architecture
//!
//! ```text
...
//!       ▼
//! ┌─────────────────────────────────────────┐
//! │ HNSW Graph (navigates using PQ distance)│
//! │   Layer 2:  A ------- B                 │
//! │   Layer 1:  A -- C -- B -- D            │
//! │   Layer 0:  A-E--C-F--B-G--D-H          │
//! └─────────────────────────────────────────┘
//!       │ (graph edges stored, PQ codes stored)
//!       │ (full embeddings NOT stored - memory savings!)
//!       ▼
//!    Top-k results (ranked by asymmetric PQ distance)
//! ```
//!
...
//! # Trade-offs
//!
//! - **Memory**: HNSW-PQ uses ~7x less memory than HNSW
//! - **Search**: HNSW-PQ is slightly slower due to PQ distance computation
//! - **Recall**: HNSW-PQ has slightly lower recall due to quantization error
//! - **Build**: HNSW-PQ requires full embeddings at build time (for graph construction),
//!   but only stores PQ codes afterward

use super::types::ProofId;
use crate::distance::euclidean_distance_sq;
use crate::embedder::Embedding;
use crate::pq::{DistanceTable, PqConfig, PqError, ProductQuantizer};
...
pub struct HnswPqConfig {
    ...
    pub seed: u64,
    /// PQ configuration for embedding compression
    pub pq: PqConfig,
}
...
impl HnswPqConfig {
    ...
    pub fn validate(&self, dim: usize) -> Result<(), HnswPqError> {
        if self.m == 0 {
            return Err(HnswPqError::InvalidConfig("m must be > 0".into()));
        }
        if self.m0 < self.m {
            return Err(HnswPqError::InvalidConfig("m0 must be >= m".into()));
        }
        if self.ef_construction < self.m {
            return Err(HnswPqError::InvalidConfig(
                "ef_construction must be >= m".into(),
            ));
        }
        self.pq.validate(dim).map_err(HnswPqError::PqError)?;
        Ok(())
    }
}

/// HNSW-PQ specific errors
#[derive(Debug, Clone)]
pub enum HnswPqError {
...
impl std::fmt::Display for HnswPqError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            HnswPqError::InvalidConfig(s) => write!(f, "Invalid HNSW-PQ config: {}", s),
            HnswPqError::DimensionMismatch { expected, got } => {
                write!(f, "Dimension mismatch: expected {}, got {}", expected, got)
            }
            HnswPqError::ProofNotFound(id) => write!(f, "Proof not found: {}", id),
            HnswPqError::MissingEmbedding(id) => write!(f, "Missing embedding for proof: {}", id),
            HnswPqError::PqError(e) => write!(f, "PQ error: {}", e),
        }
    }
}

impl std::error::Error for HnswPqError {}

impl From<PqError> for HnswPqError {
    fn from(e: PqError) -> Self {
        HnswPqError::PqError(e)
    }
}

/// A node in the HNSW-PQ graph
#[derive(Debug, Clone, Serialize, Deserialize)]
struct HnswPqNode {
...
/// HNSW-PQ hybrid index for memory-efficient graph-based search
///
/// Uses HNSW graph navigation with PQ-compressed embeddings to achieve
/// both fast O(log N) search and ~48x memory compression.
#[derive(Debug, Serialize, Deserialize)]
pub struct ProofCorpusHnswPq {
    /// Product quantizer for encoding/decoding
    quantizer: ProductQuantizer,
    /// PQ codes for each entry (compressed embeddings)
    codes: Vec<Vec<u8>>,
    /// Graph nodes with connectivity information
    nodes: Vec<HnswPqNode>,
...
impl ProofCorpusHnswPq {
    /// Build an HNSW-PQ index from a ProofCorpus
    ///
    /// Note: Requires full embeddings at build time for graph construction,
    /// but only stores PQ codes afterward.
    pub fn build(corpus: &super::storage::ProofCorpus) -> Result<Option<Self>, HnswPqError> {
        Self::build_with_config(corpus, HnswPqConfig::default())
    }
    ...
    pub fn build_with_config(
        ...
        config: HnswPqConfig,
    ) -> Result<Option<Self>, HnswPqError> {
        // Collect embeddings
        ...
        // Need enough data for PQ training
        if entries_with_embeddings.len() < config.pq.num_centroids {
            return Err(PqError::InsufficientData(format!(
                "Need at least {} embeddings for PQ training, got {}",
        ...
        let embeddings: Vec<_> = entries_with_embeddings
            ...
            .map(|(_, emb)| emb.clone())
            .collect();
        let quantizer = ProductQuantizer::train(&embeddings, config.pq.clone())?;
        ...
        // Build the graph using full embeddings, then store only PQ codes
    ...
    fn insert_with_temp_embeddings(
        ...
        if embedding.dim != self.dim {
            return Err(HnswPqError::DimensionMismatch {
                expected: self.dim,
                got: embedding.dim,
            });
        }
    ...
    ///
    /// Uses asymmetric PQ distance: query is not quantized, but graph nodes use PQ codes.
    pub fn find_similar_with_ef(
        ...
        // Precompute distance table for asymmetric PQ distance
        let distance_table = self.quantizer.build_distance_table(query);

        // Navigate from entry point down to layer 1
        let mut current_ep = ep;
        for layer in (1..=self.max_layer).rev() {
            current_ep = self.search_layer_single_pq(&distance_table, current_ep, layer);
        }

        // Search layer 0 with ef candidates
        let candidates = self.search_layer_pq(&distance_table, vec![current_ep], ef.max(k), 0);
    ...
    /// Measure recall compared to exact HNSW search
    ///
    /// Samples `num_samples` queries and compares HNSW-PQ results to brute-force.
    ...
    pub fn memory_stats(&self) -> HnswPqMemoryStats {
        // PQ codes: num_entries * num_subspaces bytes
        let codes_bytes = self.codes.len() * self.config.pq.num_subspaces;

        // PQ codebooks: num_subspaces * num_centroids * subspace_dim * 4 bytes
        let subspace_dim = self.dim / self.config.pq.num_subspaces;
        let codebook_bytes =
            self.config.pq.num_subspaces * self.config.pq.num_centroids * subspace_dim * 4;
        ...
        HnswPqMemoryStats {
            ...
            codebook_bytes,
            graph_bytes,
            total_bytes: codes_bytes + codebook_bytes + graph_bytes,
            full_embedding_bytes,
            compression_ratio: if codes_bytes + codebook_bytes > 0 {
                full_embedding_bytes as f64 / (codes_bytes + codebook_bytes) as f64
            } else {
                0.0
            },
        }
    ...
    pub fn graph_stats(&self) -> HnswPqGraphStats {
    ...
    /// Search single layer using PQ distance (for query)
    fn search_layer_single_pq(
        &self,
    ...
    /// Search layer using PQ distance (for query)
    fn search_layer_pq(
        &self,
        distance_table: &DistanceTable,
        entry_points: Vec<usize>,
    ...
    /// Save the HNSW-PQ index to a JSON file
    ...
    /// Load an HNSW-PQ index from a JSON file
...
/// Memory statistics for HNSW-PQ index
#[derive(Debug, Clone)]
pub struct HnswPqMemoryStats {
    /// Number of indexed entries
...
/// Graph statistics for HNSW-PQ index
#[derive(Debug, Clone)]
pub struct HnswPqGraphStats {
...
mod tests {
    ...
    #[test]
    fn test_hnsw_pq_config_default() {
    ...
    #[test]
    fn test_hnsw_pq_config_presets() {
        let fast = HnswPqConfig::fast();
        let high_recall = HnswPqConfig::high_recall();
        let compact = HnswPqConfig::compact();
    ...
    #[test]
    fn test_hnsw_pq_config_validation() {
        let invalid_m = HnswPqConfig {
            m: 0,
            ..Default::default()
        };
        assert!(invalid_m.validate(EMBEDDING_DIM).is_err());

        let invalid_m0 = HnswPqConfig {
            m: 16,
            m0: 8,
            ..Default::default()
        };
    ...
    fn test_build_empty_corpus() {
        ...
        let result = ProofCorpusHnswPq::build(&corpus);
        assert!(result.is_ok());
    ...
    fn test_build_insufficient_data() {
        ...
        // Only 10 entries, but PQ needs 64+ for training
        for i in 0..10 {
    ...
    fn test_build_and_search() {
        ...
        // Need enough for PQ training (64 centroids by default for fast config)
        for i in 0..100 {
            let result = make_result(&format!("prop_{}", i));
            let embedding = random_embedding(i as u64);
            corpus.insert_with_embedding(&result, embedding);
    ...
    fn test_memory_compression() {
        ...
        // PQ codes should be much smaller than full embeddings
        assert!(
            stats.codes_bytes < stats.full_embedding_bytes,
            "PQ codes ({}) should be smaller than full embeddings ({})",
            stats.codes_bytes,
            stats.full_embedding_bytes
        );

        // For small corpora, codebook overhead is significant, but codes are still compressed.
        // Full compression benefit shows at larger scale.
        // With 200 entries: full = 200 * 96 * 4 = 76800 bytes
        // PQ codes = 200 * 8 = 1600 bytes (48x smaller)
        // But codebook = 8 subspaces * 64 centroids * 12 dims * 4 bytes = 24576 bytes
        // So compression ratio = 76800 / (1600 + 24576) = 2.9x for small corpus
        // For 10000 entries: full = 3.84 MB, codes = 80KB, codebook still 24KB -> ~37x
        assert!(
            stats.compression_ratio > 1.5,
            "Expected compression ratio > 1.5, got {:.2} (codes={}, codebook={}, full={})",
        ...
        // Codes alone should show ~48x compression vs full embeddings
        let code_only_ratio = stats.full_embedding_bytes as f64 / stats.codes_bytes as f64;
        assert!(
            code_only_ratio > 40.0,
            "Code-only compression should be ~48x, got {:.1}x",
            code_only_ratio
        );
    }
    ...
    fn test_save_load() {
        ...
        let path = temp_dir.join("test_hnsw_pq_save_load.json");

        // Save
        index.save_to_file(&path).unwrap();

        // Load
        let loaded = ProofCorpusHnswPq::load_from_file(&path).unwrap();
    ...
    fn test_load_if_exists() {
        let path = std::env::temp_dir().join("test_hnsw_pq_nonexistent.json");

        // Should return None for non-existent file
        let result = ProofCorpusHnswPq::load_if_exists(&path).unwrap();
