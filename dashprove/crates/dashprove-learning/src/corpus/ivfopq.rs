//! IVFOPQ (Inverted File with Optimized Product Quantization) for billion-scale search
//!
//! IVFOPQ combines coarse quantization (IVF) with optimized product quantization (OPQ):
//!
//! - **Coarse Quantization (IVF)**: Partitions vectors into `nlist` Voronoi cells
//! - **Optimized Quantization (OPQ)**: Rotates residuals before PQ encoding to reduce error
//! - **Inverted Index**: Stores vectors grouped by their assigned cell
//!
//! Compared to IVFPQ, IVFOPQ provides 10-30% lower quantization error at the cost of
//! storing and applying an orthogonal rotation matrix.
//!
//! # Architecture
//!
//! ```text
//! Training:
//!   Embeddings → K-means (nlist centroids) → Compute residuals → OPQ on residuals
//!
//! Indexing:
//!   embedding → find nearest cell → compute residual → OPQ rotate + encode → store
//!
//! Search:
//!   query → find nprobe cells → query residual → rotate → PQ distance → top-k
//! ```
//!
//! # When to Use IVFOPQ
//!
//! Use IVFOPQ instead of IVFPQ when:
//! - You need better recall/accuracy at the same compression level
//! - Corpus size > 100,000 entries
//! - Sub-linear search time is needed
//! - Slight extra memory for rotation matrix is acceptable
//!
//! # Example
//!
//! ```ignore
...
//!
//! // Compare quantization error with IVFPQ
...
use crate::pq::{kmeans_parallel, OpqConfig, OptimizedProductQuantizer, PqError, SimpleRng};
use crate::similarity::SimilarProof;
use serde::{Deserialize, Serialize};

/// Configuration for IVFOPQ index
...
pub struct IvfOpqConfig {
    ...
    pub nprobe: usize,
    /// OPQ configuration for residual encoding
    pub opq: OpqConfig,
...
struct IvfEntry {
    ...
    /// Proof ID
    proof_id: ProofId,
    /// OPQ-encoded residual vector
    codes: Vec<u8>,
}

/// IVFOPQ index for ProofCorpus
///
/// Combines coarse quantization (inverted file) with optimized product quantization (OPQ)
/// for efficient billion-scale approximate nearest neighbor search with reduced quantization error.
#[derive(Debug)]
pub struct ProofCorpusIvfOpq {
    ...
    /// Optimized product quantizer for residual encoding
...
impl ProofCorpusIvfOpq {
    ...
    pub fn build_with_config(
        ...
        // Need enough embeddings to train OPQ on residuals
        if n < config.opq.pq.num_centroids {
            return Err(PqError::InsufficientData(format!(
                "Need at least {} embeddings for {} OPQ centroids, got {}",
                config.opq.pq.num_centroids, config.opq.pq.num_centroids, n
            )));
        }
        ...
        // Train OPQ on residuals (learns rotation matrix for better quantization)
        let opq = OptimizedProductQuantizer::train(&residuals, config.opq.clone())?;
        ...
        for (idx, ((proof_id, _), &cell)) in entries_with_embeddings
            ...
            .enumerate()
        {
            let codes = opq.encode(&residuals[idx]);
    ...
    pub fn memory_stats(&self) -> IvfOpqMemoryStats {
        ...
        // OPQ codebooks memory
        let codebook_bytes = self.opq.codebook_size_bytes();

        // Rotation matrix memory
        let rotation_bytes = dim * dim * 4;
        ...
        let total_bytes =
            coarse_bytes + codebook_bytes + rotation_bytes + codes_bytes + list_overhead;
    ...
    pub fn find_similar_approximate(
        ...
        for cell_idx in cells {
            ...
            // Build distance table for this cell (rotation is applied internally by OPQ)
            let distance_table = self.opq.build_distance_table(&query_residual_emb);

            // Score all entries in this cell
            for entry in &self.inverted_lists[cell_idx] {
                let dist = self.opq.distance_with_table(&distance_table, &entry.codes);
    ...
    /// Measure quantization error of the OPQ encoding
    ///
    ...
    pub fn measure_quantization_error(
        ...
        for i in (0..entries_vec.len()).step_by(step) {
            ...
            if let Some(ref embedding) = entry.embedding {
                ...
                // Encode and decode
                let codes = self.opq.encode(&residual_emb);
                let reconstructed = self.opq.decode(&codes);
    ...
    /// Save the index configuration to files
    ///
    /// Saves configuration, coarse centroids, and OPQ quantizer.
    ...
    pub fn save_config<P: AsRef<std::path::Path>>(
        ...
        // Save OPQ quantizer
        self.opq.save_to_file(quantizer_path)?;
    ...
    pub fn load_and_rebuild<P: AsRef<std::path::Path>>(
        ...
        let saveable: IvfOpqSaveable = crate::io::read_json(&config_path)?;

        // Load OPQ quantizer
        let opq = OptimizedProductQuantizer::load_from_file(quantizer_path)?;
        ...
        for entry in corpus.entries() {
            if let Some(ref embedding) = entry.embedding {
                ...
                // Encode residual with OPQ
                let codes = opq.encode(&residual_emb);
...
/// Saveable portion of IVFOPQ index (excludes inverted lists which can be rebuilt)
#[derive(Debug, Clone, Serialize, Deserialize)]
struct IvfOpqSaveable {
    config: IvfOpqConfig,
    coarse_centroids: Vec<Vec<f32>>,
    dim: usize,
}

/// Memory usage statistics for IVFOPQ index
#[derive(Debug, Clone)]
pub struct IvfOpqMemoryStats {
    /// Number of indexed entries
...
// Re-export IvfListStats from ivfpq since it's the same structure
pub use super::ivfpq::IvfListStats;
...
mod tests {
    ...
    #[test]
    fn test_ivfopq_config_default() {
        let config = IvfOpqConfig::default();
    ...
    #[test]
    fn test_ivfopq_config_presets() {
        let small = IvfOpqConfig::for_small_corpus();
        let medium = IvfOpqConfig::for_medium_corpus();
        let large = IvfOpqConfig::for_large_corpus();
        let huge = IvfOpqConfig::for_huge_corpus();
    ...
    #[test]
    fn test_ivfopq_config_auto() {
        let config_1k = IvfOpqConfig::auto(1_000);
        let config_50k = IvfOpqConfig::auto(50_000);
        let config_500k = IvfOpqConfig::auto(500_000);
        let config_5m = IvfOpqConfig::auto(5_000_000);
    ...
    #[test]
    fn test_ivfopq_config_validation() {
        // nlist = 0 should fail
        let config_nlist_zero = IvfOpqConfig {
            nlist: 0,
            ..Default::default()
        };
        ...
        let config_nprobe_zero = IvfOpqConfig {
            nprobe: 0,
            ..Default::default()
        };
        ...
        let config_nprobe_exceeds = IvfOpqConfig {
            nprobe: 300,
            nlist: 256,
            ..Default::default()
        };
    ...
    fn test_build_empty_corpus() {
        ...
        let result = ProofCorpusIvfOpq::build(&corpus);
        assert!(result.is_ok());
    ...
    fn test_build_insufficient_embeddings() {
        ...
        // Insert fewer embeddings than needed for OPQ centroids
        for i in 0..10 {
    ...
    fn test_memory_stats() {
        ...
        assert!(stats.rotation_bytes > 0); // OPQ has rotation matrix
    ...
    fn test_save_and_load() {
        ...
        let config_path = dir.path().join("ivfopq_config.json");
        let quantizer_path = dir.path().join("ivfopq_opq.json");
        ...
        // Load and rebuild
        let loaded = ProofCorpusIvfOpq::load_and_rebuild(&corpus, &config_path, &quantizer_path)
            .unwrap()
            .unwrap();
