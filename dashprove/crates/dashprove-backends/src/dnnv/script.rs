//! DNNV script generation and output parsing
...
/// Generate a DNNV verification script
pub fn generate_dnnv_script(spec: &TypedSpec, config: &DnnvConfig) -> Result<String, BackendError> {
    ...
    let verifier = config.verifier.as_str();

    Ok(format!(
        r#"#!/usr/bin/env python3
"""
DNNV (Deep Neural Network Verification) framework script
Generated by DashProve

DNNV provides a unified interface to multiple DNN verifiers.
"""
...
try:
    ...
    print(f"DNNV_ERROR: Missing dependency: {{e}}")
    print("DNNV_ERROR: Install with: pip install dnnv")
    sys.exit(1)

def load_network(model_path: str):
    """Load network using DNNV."""
    try:
        return parse(model_path)
    except Exception as e:
        print(f"DNNV_ERROR: Failed to load network: {{e}}")
        return None
...
def main():
    ...
    if network is None:
        print("DNNV_INFO: Using synthetic test network")
        network = create_test_network()
        input_dim = 2
        output_dim = 2
    else:
        input_dim = 2  # Simplified
        output_dim = 2
    ...
    for i, x in enumerate(test_inputs):
        try:
            ...
            # Run DNNV verification
            try:
                from dnnv import Verifier
                dnnv_verifier = Verifier.from_name(verifier)
                result = dnnv_verifier.verify(
                    network,
                    input_lb.reshape(1, -1),
                    input_ub.reshape(1, -1),
                    true_label
                )
                is_verified = result.is_safe
                ...
                print(f"DNNV_INFO: DNNV API unavailable, using bounds check: {{de}}")
                import torch
                with torch.no_grad():
    ...
    print("DNNV_RESULT_END")

    print(f"\\nDNNV_SUMMARY: Verified {{verified_count}}/{{total_count}} ({{verification_rate:.2%}})")

    if verification_rate >= 0.99:
        print("DNNV_STATUS: VERIFIED")
    elif verification_rate >= 0.90:
        print("DNNV_STATUS: PARTIALLY_VERIFIED")
    else:
        print("DNNV_STATUS: NOT_VERIFIED")
...
/// Parse DNNV output
pub fn parse_dnnv_output(
    stdout: &str,
    stderr: &str,
) -> (VerificationStatus, Option<StructuredCounterexample>) {
    if stdout.contains("DNNV_ERROR:") || stderr.contains("DNNV_ERROR:") {
        let error_msg = stdout
            .lines()
            .chain(stderr.lines())
            .find(|l| l.contains("DNNV_ERROR:"))
            .map(|l| l.replace("DNNV_ERROR:", "").trim().to_string())
            .unwrap_or_else(|| "Unknown DNNV error".to_string());
        return (VerificationStatus::Unknown { reason: error_msg }, None);
    }

    if let Some(json_str) = extract_json_result(stdout, "DNNV_RESULT_START", "DNNV_RESULT_END") {
        if let Ok(result) = serde_json::from_str::<serde_json::Value>(&json_str) {
            let verification_rate = result["verification_rate"].as_f64().unwrap_or(0.0);

            let counterexample = build_nn_counterexample("DNNV", &result, verification_rate);
    ...
    if stdout.contains("DNNV_STATUS: VERIFIED") {
        (VerificationStatus::Proven, None)
    } else if stdout.contains("DNNV_STATUS: PARTIALLY_VERIFIED") {
        (
            VerificationStatus::Partial {
    ...
    } else if stdout.contains("DNNV_STATUS: NOT_VERIFIED") {
        (VerificationStatus::Disproven, None)
    } else {
        (
            VerificationStatus::Unknown {
                reason: "Could not parse DNNV output".to_string(),
            },
            None,
        )
    }
