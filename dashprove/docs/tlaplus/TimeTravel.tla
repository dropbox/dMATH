...
MonotonicHighWaterMark ==
    [][highWaterMark' >= highWaterMark]_vars
...
NoSequenceReuse ==
    \A seq \in 1..highWaterMark:
        [][seq \in DOMAIN stateHistory => stateHistory'[seq] = stateHistory[seq]]_vars
...
(* Note: This invalidates future history (advanced feature)                *)
(***************************************************************************)
...
ForkFromPast(delta) ==
    ...
    /\ LET newSeq == cursor + 1
           newState == currentState + delta
       IN /\ cursor' = newSeq
          /\ highWaterMark' = newSeq  \* Truncates future
          /\ stateHistory' = [s \in 0..newSeq |->
                IF s = newSeq THEN newState
                ELSE IF s <= cursor THEN stateHistory[s]
                ELSE 0]  \* Future invalidated
          /\ currentState' = newState
          /\ mode' = "executing"
...
ReconstructionProperty ==
    \A seq \in 1..highWaterMark:
        \E state \in Nat: stateHistory[seq] = state
